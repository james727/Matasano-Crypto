...
class mitm_haxor(object):
    ...
    def accept_first_session(self, counterpart):
        self.p = counterpart.p
        self.g = counterpart.g
        self.pk1 = counterpart.get_public_key()

    def accept_second_session(self, counterpart):
        self.pk2 = counterpart.get_public_key()
...
class diffie_party(object):
    ...
    def get_message(self, counterpart):
        ...
        return pks_unpadding(plain_text, BLOCK_SIZE)
...
def haxored_communication():
    ...
    # key transfer
    M.accept_first_session(A)
    B.accept_session(M)
    M.accept_second_session(B)
    A.generate_session_key(M.get_public_key())
    M.generate_session_key(A.get_public_key)
    ...
    # decrypt
    print M.get_message(A)
    print M.get_message(B)
